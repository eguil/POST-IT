...
;
; @keyword CHARSIZE
; The character size of the color bar annotations. Default is 1.0.
;
; @keyword COLOR
; The color index of the bar outline and characters. Default
;                 is ncolors - 1 + bottom.
;
; @keyword DISCRET
; Vecteur contenant les incices des couleurs a tracer en barre
; de couleur. On obtient ainsi une barre de couleur discrete ne comportant que
; les couleurs specifiees ds l'ordre ou elles apparaissent ds le vecteur.
;
; @keyword DIVISIONS
...
;
; @keyword FORMAT
; The format of the bar annotations. Default is '(F6.2)'.
;
; @keyword MAX
; The maximum data value for the bar annotation. Default is
;                 NCOLORS-1.
;
; @keyword MIN
; The minimum data value for the bar annotation. Default is 0.
;
; @keyword NCOLORS
...
;
; @keyword TITLE
; This is title for the color bar. The default is to have
;                 no title.
;
; @keyword TOP
...
;
; @keyword VERTICAL
; Setting this keyword give a vertical color bar. The default
...
;   * check parameters
;
; - fplod 20091118T080732Z
;
;   * extract from tvnplot.pro
;
...
;
; @version
; $Id: colorbartv.pro 206 2010-01-26 10:33:28Z pinsard $
;
;-
PRO colorbartv $
    , BOTTOM=bottom $
...
IF KEYWORD_SET(vertical) THEN BEGIN
   IF KEYWORD_SET(discret) THEN BEGIN
       facteur=256/n_elements(discret)
    discret=reform(replicate(1,facteur) # discret,facteur*n_elements(discret))
       bar = REPLICATE(1B,10) # discret
   ENDIF ELSE BEGIN
       bar = REPLICATE(1B,10) # BINDGEN(256)
   ENDELSE
   IF N_ELEMENTS(position) EQ 0 THEN BEGIN
    position = [0.88, 0.15, 0.95, 0.95]
   ENDIF
ENDIF ELSE BEGIN
   IF KEYWORD_SET(discret) THEN BEGIN
       facteur=256/n_elements(discret)
    discret=reform(replicate(1,facteur) # discret,facteur*n_elements(discret))
       bar = discret # REPLICATE(1B,10)
   ENDIF ELSE BEGIN
       bar = BINDGEN(256) # REPLICATE(1B, 10)
   ENDELSE
   IF N_ELEMENTS(position) EQ 0 THEN BEGIN
    position = [0.15, 0.88, 0.95, 0.95]
   ENDIF
ENDELSE

   ; Scale the color bar.
IF NOT KEYWORD_SET(discret) THEN BEGIN
