;+
;
; compute point-wise slope of linear regression of two 3D (x,y,t) fields
;
;
...
;
; @version
; $Id: make_linfit.pro 235 2010-06-25 08:46:06Z ericg $
;
;-
FUNCTION make_linfit, file_name, ncdf_db $
         , BOXZOOM=boxzoom $
         , TIME_1=time_1 $
         , TIME_2=time_2 $
         , ALL_DATA=all_data
...
;
 ...
 usage='result=make_linfit(file_name, ncdf_db ' $
         + ', BOXZOOM=boxzoom ' $
         + ', TIME_1=time_1 ' $
         + ', TIME_2=time_2 ' $
         + ', ALL_DATA=all_data)'
      ...
      FOR idx = 0, nxa-1 DO FOR idy = 0, nya-1 DO BEGIN
         ...
         IF data1[idx, idy, 0] LT valmask/10. THEN BEGIN
            ...
            ; compute regression for value above (p - default) or below (m) linfit_sep
               CASE linfit_map OF
                  'm': idt = where (x2 LE linfit_sep)
                  ELSE: idt = where (x2 GE linfit_sep)
               ENDCASE
...
;          ENDIF
  ...
  ; only take points where correlation larger than 0.2
   ...
   print, '    Linfit min/max pt_corr ', min(pt_corr), max(pt_corr)
