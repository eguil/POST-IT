;+
;
; make divergence of vector field
;
;
...
;
; @version
; $Id: make_wdiv.pro 206 2010-01-26 10:33:28Z pinsard $
;
;-
FUNCTION make_wdiv, file_name, ncdf_db $
         , BOXZOOM=boxzoom $
         , TIME_1=time_1 $
         , TIME_2=time_2 $
         , ALL_DATA=all_data $
...
;
 ...
 usage='result=make_wdiv(file_name, ncdf_db ' $
         + ', BOXZOOM=boxzoom ' $
         + ', TIME_1=time_1 ' $
         + ', TIME_2=time_2 ' $
         + ', ALL_DATA=all_data ' $
...
;   for ORCA C grid wdiv = div(var_x, var_y)

   grille,mask,glam,gphi,gdep,nx,ny,nz,premierx,premiery,premierz,dernierx,derniery,dernierz

   IF jpt EQ 1 THEN BEGIN
      wdiv = (var_x.data - shift(var_x.data, -1, 0))/e1t[premierx:dernierx, premiery:derniery] + (var_y.data-shift(var_y.data, 0, -1))/e2t[premierx:dernierx, premiery:derniery]
   ENDIF ELSE BEGIN
      ...
      wdiv = (var_x.data - shift(var_x.data, -1, 0, 0))/e1t3d + (var_y.data-shift(var_y.data, 0, -1, 0))/e2t3d
   ENDELSE

   field = {name: '', data: wdiv, legend: '', units: '', origin: '', dim: 0, direc:''}
